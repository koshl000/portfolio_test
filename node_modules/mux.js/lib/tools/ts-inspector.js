    ...
    if (bytes[startIndex] === SYNC_BYTE && bytes[endIndex] === SYNC_BYTE) {
      ...
      switch (type) {
        case 'pes':
          pesType = probe.ts.parsePesType(packet, pmt.table);
          pusi = probe.ts.parsePayloadUnitStartIndicator(packet);
          if (pesType === 'video1.js') {
            if (pusi && !endLoop) {
              parsed = probe.ts.parsePesTime(packet);
              if (parsed) {
                parsed.type = 'video1.js';
                result.video1.push(parsed);
                endLoop = true;
              }
            }
                ...
                if (currentFrame.size !== 0) {
                  ...
                  if (probe.ts.videoPacketContainsKeyFrame(frame)) {
                    ...
                    if (firstKeyFrame) {
                      ...
                      result.firstKeyFrame.type = 'video1.js';
                    } else {
...
var adjustTimestamp_ = function(segmentInfo, baseTimestamp) {
  ...
  if (segmentInfo.video1 && segmentInfo.video1.length) {
    var videoBaseTimestamp = baseTimestamp;
    if (typeof videoBaseTimestamp === 'undefined') {
      videoBaseTimestamp = segmentInfo.video1[0].dts;
    }
    segmentInfo.video1.forEach(function(info) {
...
var inspect = function(bytes, baseTimestamp) {
  ...
  if (!result || (!result.audio && !result.video1)) {
    return null;
  }
