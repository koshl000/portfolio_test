...
Play HLS, DASH, and future HTTP streaming protocols with video1.js, even where they're not
natively supported.

Included in video1.js 7 by default! See the [video1.js 7 blog post](https://blog.videojs.com/video-js-7-is-here/)
...
## Getting Started
This library is included in video1.js 7 by default, if you are using an older version of video1.js then
get a copy of [videojs-http-streaming](#installation) and include it in your page along with video1.js:
...
- Tight integration with video1.js and a philosophy of exposing as much
...
video1.js plugin, we'd recommend you avoid it. Your plugin won't work
with videos that don't use videojs-http-streaming and the best plugins
work across all the media types that video1.js supports. If you're
...
### Events
Standard HTML video events are handled by video1.js automatically and
are triggered on the player object.

#### loadedmetadata

Fired after the first segment is downloaded for a playlist. This will not happen
until playback if video1.js's `metadata` setting is `none`
...
__NOTE__: The `debug` level is only available in video1.js v6.6.0+. With earlier versions of
video1.js, no debug messages will be logged to console.
